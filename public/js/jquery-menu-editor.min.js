function MenuEditor(e, s) {
    ...
    function d() { i[0].reset(), (c = c.iconpicker(a)).iconpicker("setIcon", "empty"), r.attr("disabled", !0), n = null }

    function p(e) { return $("<a>").addClass(e.classCss).addClass("clickable").attr("href", "#").html(e.text) }

    function u() {
        var e = $("<div>").addClass("btn-group float-right"),
            s = p({ classCss: "btn btn-primary btn-sm btnEdit", text: l.labelEdit }),
            t = p({ classCss: "btn btn-danger btn-sm btnRemove", text: l.labelRemove }),
            n = p({ classCss: "btn btn-secondary btn-sm btnUp btnMove", text: '<i class="fas fa-angle-up clickable"></i>' }),
            o = p({ classCss: "btn btn-secondary btn-sm btnDown btnMove", text: '<i class="fas fa-angle-down clickable"></i>' }),
            i = p({ classCss: "btn btn-secondary btn-sm btnOut btnMove", text: '<i class="fas fa-level-down-alt clickable"></i>' }),
            r = p({ classCss: "btn btn-secondary btn-sm btnIn btnMove", text: '<i class="fas fa-level-up-alt clickable"></i>' });
        return e.append(n).append(o).append(r).append(i).append(s).append(t), e
    }

    function f(e) { $("<span>").addClass("sortableListsOpener " + s.opener.openerClass).css(s.opener.openerCss).on("mousedown touchstart", function(e) { var t = $(this).closest("li"); return t.hasClass("sortableListsClosed") ? t.iconOpen(s) : t.iconClose(s), !1 }).prependTo(e.children("div").first()), e.hasClass("sortableListsOpen") ? e.iconOpen(s) : e.iconClose(s) }
    ...
    t.sortableLists(l.listOptions), c.on("change", function(e) { i.find("[name=icon]").val(e.icon) }), t.on("click", ".btnRemove", function(s) {
        ...
        e.preventDefault(),
            function(e) {
                var s = e.data();
                $.each(s, function(e, s) { i.find("[name=" + e + "]").val(s) }), i.find(".item-menu").first().focus(), s.hasOwnProperty("icon") ? c.iconpicker("setIcon", s.icon) : c.iconpicker("setIcon", "empty");
        ...
        if (l.length > 0)
            if ((n = l.children("ul")).length > 0) n.append(s);
                ...
                l.append(n), n.append(s), l.addClass("sortableListsOpen"), f(l)
            }
        MenuEditor.updateButtons(t), t.updateLevels()
    }), this.setForm = function(e) { i = e }, this.getForm = function() { return i }, this.setUpdateButton = function(e) {
        (r = e).attr("disabled", !0), n = null
    }, this.getUpdateButton = function() { return r }, this.getCurrentItem = function() { return n }, this.update = function() {
        var e = this.getCurrentItem();
        if (null !== e) {
            var s = e.data("icon");
            i.find(".item-menu").each(function() { e.data($(this).attr("name"), $(this).val()) }), e.children().children("i").removeClass(s).addClass(e.data("icon")), e.find("span.txt").first().text(e.data("text")), d()
        }
    }, this.add = function() {
        var e = {};
        i.find(".item-menu").each(function() { e[$(this).attr("name")] = $(this).val() });
        var s = u(),
            l = $("<span>").addClass("txt").text(e.text),
            n = $("<i>").addClass(e.icon),
            o = $("<div>").css({ overflow: "auto" }).append(n).append("&nbsp;").append(l).append(s),
            r = $("<li>").data(e);
        r.addClass("list-group-item pr-0").append(o), t.append(r), MenuEditor.updateButtons(t), d()
    }, this.getString = function() { var e = t.sortableListsToJson(); return JSON.stringify(e) }, this.setData = function(e) {
        var s = Array.isArray(e) ? e : function(e) { try { var s = JSON.parse(e) } catch (e) { return console.log("The string is not a json valid."), null } return s }(e);
        if (null !== s) {
            t.empty();
            var n = function e(s, l) {
                var n = void 0 === l ? 0 : l,
                    o = 0 === n ? t : $("<ul>").addClass("pl-0").css("padding-top", "10px").data("level", n);
                return $.each(s, function(s, t) {
                    var l = void 0 !== t.children && $.isArray(t.children),
                        i = { text: "", href: "", icon: "empty", target: "_self", title: "" },
                        r = $.extend({}, t);
                    ...
                    var c = $("<div>").css("overflow", "auto"),
                        d = $("<i>").addClass(t.icon),
                        p = $("<span>").addClass("txt").append(t.text).css("margin-right", "5px"),
                        f = u();
                    c.append(d).append("&nbsp;").append(p).append(f), a.append(c), l && a.append(e(t.children, n + 1)), o.append(a)
                }), o
            }(s);
    ...
    e.fn.sortableLists = function(s) {
        var t = e("body").css("position", "relative"),
            ...
            c = e("<span />").addClass("sortableListsOpener " + n.opener.openerClass).css(n.opener.openerCss).on("mousedown touchstart", function(s) { var t = e(this).closest("li"); return t.hasClass("sortableListsClosed") ? L(t) : E(t), !1 });
        ...
        var d = { isDragged: !1, isRelEFP: null, oEl: null, rootEl: null, cEl: null, upScroll: !1, downScroll: !1, pX: 0, pY: 0, cX: 0, cY: 0, isAllowed: !0, e: { pageX: 0, pageY: 0, clientX: 0, clientY: 0 }, doc: e(document), win: e(window) };
        if (n.opener.active) {
            ...
            if (!(!1 !== d.isDragged || n.ignoreClass && t.hasClass(n.ignoreClass))) {
                ...
                l[0] && (n.onDragStart(s, l), function(s, t, l) {
                    ...
                    var a = parseInt(t.css("margin-top")),
                        ...
                        v = t.offset(),
                        b = t.innerHeight();
                    d.rootEl = { el: l, offset: l.offset(), rootElClass: l.attr("class") }, d.cEl = { el: t, mT: a, mL: f, mB: c, mR: h, offset: v }, d.cEl.xyOffsetDiff = { X: s.pageX - d.cEl.offset.left, Y: s.pageY - d.cEl.offset.top }, d.cEl.el.addClass("sortableListsCurrent " + n.currElClass), t.before(i);
                    var g = d.placeholderNode = e("#sortableListsPlaceholder");
                    t.css({ width: t.width(), position: "absolute", top: v.top - a, left: v.left - f }).prependTo(o), g.css({ display: "block", height: b }), r.css("height", b), d.doc.on("mousemove touchmove", p).on("mouseup touchend touchcancel", u)
                }(s, l, a))
            }
        });
        ...
        function p(s) {
            if (d.isDragged) {
                var t = d.cEl,
                    l = d.doc,
                    o = d.win;
                "touchmove" === s.type && v(s), s.pageX || function(e) { e.pageY = d.pY, e.pageX = d.pX, e.clientY = d.cY, e.clientX = d.cX }(s), l.scrollTop() > d.rootEl.offset.top - 10 && s.clientY < 50 ? d.upScroll ? (s.pageY = s.pageY - n.scroll, e("html, body").each(function(s) { e(this).scrollTop(e(this).scrollTop() - n.scroll) }), f(s)) : function(e) {
                        if (d.upScroll) return;
                        d.upScroll = setInterval(function() { d.doc.trigger("mousemove") }, 50)
                    }() : l.scrollTop() + o.height() < d.rootEl.offset.top + d.rootEl.el.outerHeight(!1) + 10 && o.height() - s.clientY < 50 ? d.downScroll ? (s.pageY = s.pageY + n.scroll, e("html, body").each(function(s) { e(this).scrollTop(e(this).scrollTop() + n.scroll) }), f(s)) : function(e) {
                        if (d.downScroll) return;
                        d.downScroll = setInterval(function() { d.doc.trigger("mousemove") }, 50)
                    ...
                    function(e, s) {
                        var t = s.cEl;
                        t.el.css({ top: e.pageY - t.xyOffsetDiff.Y - t.mT, left: e.pageX - t.xyOffsetDiff.X - t.mL })
                    }(s, d)
            }
        }
        ...
        function u(s) {
            ...
            "touchend" !== s.type && "touchcancel" !== s.type || v(s), "block" == o.display && l.length && d.isAllowed ? (i = l, a = !0) : (i = d.placeholderNode, a = !1), offset = i.offset(), t.el.animate({ left: offset.left - d.cEl.mL, top: offset.top - d.cEl.mT }, 250, function() {
                ! function(e) {
                    var s = e.el[0].style;
                    e.el.removeClass(n.currElClass + " sortableListsCurrent"), s.top = "0", s.left = "0", s.position = "relative", s.width = "auto"
                }(t), i.after(t.el[0]), i[0].style.display = "none", o.display = "none", l.remove(), f.removeAttr("id").removeClass(n.hintWrapperClass), f.length && f.prev("div").prepend(c.clone(!0)), a ? d.placeholderNode.slideUp(150, function() { d.placeholderNode.remove(), y(), n.onChange(t.el), n.complete(t.el), d.isDragged = !1 }) : (d.placeholderNode.remove(), y(), n.complete(t.el), d.isDragged = !1)
            }), h(d), d.doc.unbind("mousemove touchmove", p).unbind("mouseup touchend touchcancel", u)
        }

        function f(e) { d.pY = e.pageY, d.pX = e.pageX, d.cY = e.clientY, d.cX = e.clientX }

        function h(e) { clearInterval(e.upScroll), clearInterval(e.downScroll), e.upScroll = e.downScroll = !1 }

        function v(e) { e.pageX = e.originalEvent.changedTouches[0].pageX, e.pageY = e.originalEvent.changedTouches[0].pageY, e.screenX = e.originalEvent.changedTouches[0].screenX, e.screenY = e.originalEvent.changedTouches[0].screenY }
    ...
    }, e.fn.hideButtons = function(s) { for (var t = 0; t < s.length; t++) e(this).find(".btn-group:first").children(".btn" + s[t]).hide() }
